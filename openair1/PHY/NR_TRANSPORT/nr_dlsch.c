...
/* A/B switches for the cross-polar fast paths: NR_PDSCH_NO_2X4=1 and NR_PDSCH_NO_NX4=1
   force the generic per-port kernel, so either can be measured against it. Read once at
   load time -- the decision is taken per symbol, which is too hot for getenv(). */
static bool nr_pdsch_no_2x4, nr_pdsch_no_nx4;
...
static void nr_pdsch_symbol_processing(void *arg)
{
  ...
  for (int l_symbol = rdata->startSymbol; l_symbol < rdata->startSymbol + rdata->numSymbols; l_symbol++) {
    ...
    if ((rel15->nrOfLayers == 3 || rel15->nrOfLayers == 4) && num_log_ports == 4 && !nr_pdsch_no_nx4
