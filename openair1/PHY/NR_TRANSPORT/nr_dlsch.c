...
/* A/B switches for the cross-polar fast paths, read once at load time -- the decision is
   taken per symbol, which is too hot for getenv().  They disable one dispatch branch each,
   and the branches fall through to one another, so at rank 2 on aarch64:

     (neither)                    the 2x4 rank-2 specialisation
     NR_PDSCH_NO_2X4=1            the general Nx4 kernel, NOT the generic per-port one
     NR_PDSCH_NO_NX4=1            still the 2x4 specialisation
     both                         the generic per-port kernel

   At rank 3-4, and at every rank on x86 (where there is no 2x4 branch), NR_PDSCH_NO_NX4=1
   alone gives the generic kernel.  Set both to measure against the generic kernel. */
static bool nr_pdsch_no_nx4;
#if defined(__aarch64__) && !defined(__ARM_FEATURE_QRDMX)
static bool nr_pdsch_no_2x4;
#endif
static void __attribute__((constructor)) nr_pdsch_read_fastpath_env(void)
{
  nr_pdsch_no_nx4 = getenv("NR_PDSCH_NO_NX4") != NULL;
#if defined(__aarch64__) && !defined(__ARM_FEATURE_QRDMX)
  nr_pdsch_no_2x4 = getenv("NR_PDSCH_NO_2X4") != NULL;
#endif
}
#endif
...
static void nr_pdsch_symbol_processing(void *arg)
{
  ...
  for (int l_symbol = rdata->startSymbol; l_symbol < rdata->startSymbol + rdata->numSymbols; l_symbol++) {
    ...
    if (rel15->nrOfLayers == 2 && num_log_ports == 4 && !nr_pdsch_no_2x4 && nr_pdsch_2x4_usable(gNB, rel15)) {
      int pos = 0;
      int block_start, block_end;
      while (find_next_rb_block(freq_alloc->bitmap, rel15->BWPSize, &pos, &block_start, &block_end)) {
    ...
    if (rel15->nrOfLayers >= 2 && rel15->nrOfLayers <= 4 && num_log_ports == 4 && !nr_pdsch_no_nx4
        && nr_pdsch_Nx4_usable(gNB, rel15)) {
      int pos = 0;
      int block_start, block_end;
      while (find_next_rb_block(freq_alloc->bitmap, rel15->BWPSize, &pos, &block_start, &block_end)) {
